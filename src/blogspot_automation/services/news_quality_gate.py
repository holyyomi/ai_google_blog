...
class NewsQualityGate:
    ...
    def _compute_issue_specificity(cls, selected: ScoredNewsCandidate) -> int:
        ...
        specific_keywords = (
            ...
            # AI 뉴스 엔티티·사건 명사(2026-07-11): 목록이 소비자·정책 위주라
            # "오픈AI GPT-5.6 공개" 같은 고유명사 가득한 AI 뉴스가 무적중
            # 5점 안팎에 갇혔고(18건 중 16건 specificity 차단), 재탕 에버그린만
            # 게이트를 통과하는 역선택이 벌어졌다. 임계값은 그대로 두고
            # 엔티티 인식 범위만 AI 도메인으로 넓힌다.
