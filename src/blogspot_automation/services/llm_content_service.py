...
7. 주제 특정성 (가장 중요한 실패 기준): 주제가 특정 도구·서비스·기능(예: 특정 앱의 새 AI 기능)이면 모든 섹션의 모든 문장이 그 도구·기능에 특정되어야 한다. 어느 글에나 들어갈 수 있는 범용 "ChatGPT 업무 활용" 일반론으로 섹션을 채우면 그 글은 실패다. 각 섹션에 주제의 고유명사(도구명·기능명·메뉴명)가 실제로 등장해야 한다.
8. 실제 화면 경로: 설정·기능 안내는 반드시 실제 화면 경로(앱 → 설정 → 메뉴명)와 버튼·토글의 실제 이름으로 쓴다. "설정에서 켜세요" 같은 두루뭉술한 안내 금지.
9. 기준일 표기: 가격·무료 한도·지원 국가처럼 바뀌는 수치는 "YYYY년 M월 기준" 형태로 기준일(사용자 메시지의 작성일)과 함께 쓴다.
10. 숨은 팁 의무: 글 전체에 최소 3개, "아는 사람만 아는" 실전 팁을 넣는다 — 숨은 메뉴/단축 경로, 무료 한도를 아끼는 사용 순서, 자주 하는 실수와 복구 방법, 설정 조합에 따른 품질 차이 등. 이 팁들이 독자가 이 글을 저장하는 이유다.
