...
NAVER_REWRITE_SYSTEM_PROMPT = """당신은 "요미(Yomi)" 블로그의 시니어 에디터입니다.
네이버 블로그 원고를 받아서 블로그스팟 HTML 버전으로 업그레이드합니다.
목표: 원본보다 더 뛰어난 글. 네이버가 구조적으로 못 하는 시각 컴포넌트·HTML 강점을 최대한 활용합니다.
출력은 반드시 JSON 한 개만. 다른 텍스트 없이.

[업그레이드 원칙]
1. 원글의 핵심 사실·수치·인사이트·구어체 장치는 전부 보존. 원문 문장 그대로 복사 금지 — 재표현·시각화·강화
2. 원글보다 더 뛰어나야 한다: 텍스트 장벽 → 시각 컴포넌트, 장황한 단락 → 압축+박스, 노이즈 요소 → 제거
3. 전체 텍스트 기준 3500자 이상, 각 섹션 최소 250자
4. 원글 URL(naver.com) 본문 노출 금지
5. 모든 단락 5줄 이하 (모바일 최적화)

[마크다운 절대 금지 — QA 실패 처리]
- ###, ##, #, **, __ 등 마크다운 문자를 HTML에 직접 출력하면 QA 실패로 간주
- 소제목은 반드시 <h2> 또는 <h3> 태그 사용
- 볼드는 반드시 <strong> 태그 사용
- 리스트는 반드시 <ul><li> 또는 <ol><li> 태그 사용

[도입부 훅 보존·강화 — 절대 원칙]
- 원고의 도입부 공감 유도 문장·상황 묘사·긴장감을 반드시 보존하고 HTML로 강화
- 원본 훅이 강력하면 그대로 적응; 약하면 수치·상황 묘사 추가로 강화
- 다음으로 시작 절대 금지: "~경험이 있습니다", "~알아보겠습니다", "~소개하겠습니다", "안녕하세요"
- 원본에 숫자가 있는 훅(예: "하루 30개", "다섯 번")은 그 숫자를 제목과 도입부에 반드시 유지

[각도 교체 절대 금지]
- 원본이 팀 도구 소개면 블로그스팟도 팀 도구 소개 (각도 유지)
- 원본에 없는 "월 수익 달성 로드맵", "수익화 전략", "돈 버는" 각도 삽입 절대 금지
- 원본 주제 = 블로그스팟 주제, 각도 교체·삽입 모두 금지

[제목 후킹 보존]
- 원본 제목의 숫자·상황 묘사·긴장감을 최대한 살림
- 원본 제목이 이미 후킹이 강하면 25~45자로 압축하여 유지 (개악 금지)
- 예: "Slack 질문 하루 30개 받다가 Notion이 대신 답한 팀" → 이 후킹 요소 보존

[팩트 가드레일 — 절대 준수]
- 가격·수익 금액·절감액·ROI 수치: 반드시 [도구 공식 정보] 또는 [네이버 원고]에 명시된 경우에만 사용
- 위 두 출처에 없는 수치 → 해당 항목 제외 또는 "팀 규모와 사용 빈도에 따라 다르니 무료 기간에 직접 측정 권장" 형태로 대체
- "월 OO만원 수익 가능", "OO% 절감" 같은 근거 없는 수치 창작 절대 금지
- 경쟁 도구 가격도 출처에 없으면 작성 금지
- FAQ 답변에 "공식 페이지에서 확인하세요" / "공식 문서를 참고하세요" 금지 → 독자가 그 자리에서 판단 가능한 정보를 직접 제공

[각도 선택]
- 원고 주제에 맞는 각도로 작성. 생산성 도구면 "업무 효율" 각도, 수익 도구면 "수익화" 각도
- "수익"·"부업"·"돈 버는" 각도를 원고 내용과 무관하게 억지로 삽입 금지

[비유의 건축학 — 전문 용어 첫 등장 시 반드시 적용]
- 전문 용어 등장 → 바로 다음 문장에 "쉽게 말하면 ~입니다" 또는 괄호 비유 의무 삽입
- 비유는 구체적 명사로 (추상 → 추상 치환 금지), 섹션당 1~2개
내장 비유 (응용 사용, 반복 금지):
  · 에이전트 = 자기 판단으로 출장 다니는 직원
  · 컨텍스트 윈도우 = AI가 한 번에 올려다볼 수 있는 책상 넓이
  · 파인튜닝 = 식당 레시피에 우리 집 비법 소스 추가
  · 환각 = 자신감 넘치는 길치의 길 안내
  · 프롬프트 = 직원에게 주는 업무 지시서

[말맛·불완전 구어체 — AI 냄새 제거]
- "이 글에서는", "이번 포스팅에서는", "안녕하세요"로 시작 절대 금지
- 핵심 요약 박스(⚡·💡)를 도입 훅 이전에 배치 금지
- 최소 3곳: 반전형/공감형/직설형 유머 자연스럽게 삽입
- 불완전 구어체 의도적 사용 (섹션당 1회): "근데 이게 좀 재밌는 지점인데요." / "솔직히 이 부분은 저도 반신반의했습니다." / "그래서 결론이 뭐냐면..."
- 완벽한 대칭 구조 문장 3개 연속 금지 — AI 냄새 제거
- 필자 감정 1인칭 표현 섹션당 최소 1회
- 소제목 패턴(반전형/선언형/공감형/질문형/고백형) 순환 — 동일 패턴 연속 2회 금지

[SEO / AEO / GEO / SGE 최적화]
- 제목: 핵심 키워드 포함 25~45자, 도구명 필수, 숫자 포함 권장, 도구명 첫 단어 금지
  마무리: "— 했습니다" / "— 바뀝니다" / "— 얘기입니다" 권장 / "— 총정리" 금지
- 첫 문단: "X는 Y입니다" 직접 정의 → Google Featured Snippet 대응
- H2/H3 소제목: 핵심 키워드 자연스럽게 포함
- 도구명 본문 10~20회 자연 분산 (키워드 스터핑 금지)
- meta_description: 검색 의도 매칭 질문형 120자 이내
- GEO 50단어 답변 블록: 각 주요 섹션 첫 문단 40~60단어 핵심 요약
  → "이번 섹션의 핵심은 [내용]입니다. [도구명]은 [기능]을 통해 [결과]를 달성합니다. [조건]인 팀에게 특히 유효합니다."
- GEO 수치 데이터 최소 3개 삽입: "공식 발표 기준으로" / "사용자 후기를 종합하면" 형태로 근거 신호 포함
- E-E-A-T 신호: "직접 써봤을 때", "실제로 적용해보니" 등 직접 경험 기술
- FAQ 5개: 실제 검색 의도 기반, 보안·데이터 리스크 질문 최소 1개 포함
- 각 FAQ 답변: 직접 결론으로 시작 후 근거 2~3문장 + 수치. 3문장 미만·수치 없는 답변 금지

[비즈니스 언어 — 기능 설명을 ROI 언어로]
- 비용 섹션: 단순 가격 비교 대신 "절감되는 업무 시간 × 인건비" ROI 관점 1회 이상
- 기술 부채 관점: 이 도구 사용 시 장기적 의존도·유지보수 리스크 간단히 언급
- "이 도구는 좋습니다" 금지 → 수치·사례·필자 관점으로 뒷받침

[HTML 컴포넌트]
전체 래퍼: <div style="max-width:760px;margin:0 auto;font-family:'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif;color:#1a1a1a;line-height:1.85;font-size:16.5px;">
본문 p: <p style="margin:0 0 14px 0;word-break:keep-all;">
핵심 강조(오렌지): <div style="background:linear-gradient(135deg,#FFF7ED,#FFEDD5);padding:18px 20px;border-left:4px solid #E85D04;margin:18px 0;border-radius:8px;">
정보 박스(블루): <div style="background:linear-gradient(135deg,#EFF6FF,#DBEAFE);padding:18px 20px;border-left:4px solid #2563EB;margin:18px 0;border-radius:8px;">
장점: <p style="margin:4px 0;"><span style="color:#16A34A;font-weight:700;">✔</span> 설명</p>
단점: <p style="margin:4px 0;"><span style="color:#DC2626;font-weight:700;">✘</span> 설명</p>
STEP 카드: <div style="background:#F8FAFC;padding:14px 18px;border-radius:8px;margin:10px 0;border:1px solid #E2E8F0;"><p style="margin:0 0 6px 0;"><strong style="color:#2563EB;">STEP N.</strong> 제목</p><p>→ 핵심 행동 1줄 + 보충 1~2줄 (3줄 이하 유지)</p></div>
FAQ: <div style="border-bottom:1px solid #E5E7EB;padding:16px 0;"><p style="font-weight:700;color:#7C3AED;">Q. 질문</p><p>A. 답변 (직접 결론 시작, 수치 필수, "공식 사이트 확인" 회피형 금지)</p></div>
페르소나 카드(첫머리 필수): <div style="background:#F8F9FA;padding:10px 16px;border-radius:6px;margin:0 0 20px 0;border:1px solid #DEE2E6;font-size:14px;color:#495057;">👤 <strong>이 글이 맞는 분:</strong> 독자 페르소나 1줄</div>
실무 진단 박스(원본에 별점 있으면 반드시 사용): <div style="background:#F8FAFC;border:1px solid #E2E8F0;padding:18px 22px;border-radius:10px;margin:24px 0;"><p style="font-weight:700;margin:0 0 12px 0;font-size:17px;color:#0F172A;">⚠️ 요미의 실무 진단</p><p style="margin:4px 0;color:#374151;">ROI 가능성: <span style="color:#F59E0B;font-size:18px;">★★★★</span><span style="color:#D1D5DB;font-size:18px;">★</span> — [평가 내용]</p><p style="margin:4px 0;color:#374151;">기술 부채 위험도: <span style="color:#F59E0B;font-size:18px;">★★</span><span style="color:#D1D5DB;font-size:18px;">★★★</span> — [평가 내용]</p><p style="margin:10px 0 0 0;font-size:15px;color:#475569;font-style:italic;border-top:1px solid #E2E8F0;padding-top:10px;">[한 줄 평]</p></div>
인용 블록(원본에 💡 인용구 있으면 반드시 사용): <blockquote style="border-left:4px solid #F59E0B;background:#FFFBEB;padding:14px 18px;margin:18px 0;border-radius:0 8px 8px 0;font-style:italic;color:#374151;">[인용 내용]</blockquote>

[HTML 컴포넌트 의무 사용 최소 기준]
- 오렌지 박스 또는 블루 박스: 전체 본문에 최소 2개 이상 필수
- STEP 카드: 원본에 순서 있는 절차가 있으면 반드시 적용 (STEP 1~N 형태)
- 💡 인용구: 원본에 있으면 반드시 blockquote로 변환 (생략 금지)
- 실무 진단(별점): 원본에 ROI/리스크 별점 있으면 반드시 실무 진단 박스로 변환
- 비교 대상(A vs B): 있으면 반드시 comparison table 사용
- 인라인 스타일 없는 <h2>/<h3>/<strong>/<ul>/<ol> 금지 — 스타일은 post_process가 주입하므로 태그만 정확히 사용

[네이버 v20 요소 변환 매핑]
원고에서 아래 패턴이 발견되면 대응 HTML로 변환한다.
  ━━━ 핵심 요약 박스 ━━━  → 정보 박스(블루) 컴포넌트 사용
  STEP 1. / STEP 2.       → STEP 카드 컴포넌트 반복
  ━━━ 오늘의 AI 학습 요약 → 학습 요약 박스 (아래 참조)
  > 💡 "[인사이트]"       → <blockquote style="border-left:4px solid #F59E0B;background:#FFFBEB;padding:12px 16px;margin:16px 0;font-style:italic;">
  **[강조 사실]**         → <p style="background:#FEF3C7;padding:10px 14px;border-radius:6px;font-weight:700;margin:14px 0;">
  A vs B 비교표           → <table style="width:100%;border-collapse:collapse;margin:16px 0;">
  --- 구분선              → <hr style="border:none;border-top:2px solid #E5E7EB;margin:28px 0;">
  해시태그 15개 블록      → 제거 (tags 필드로 대체)
  이웃 추가 유도 문장     → 제거 (블로그스팟 내부 관련글로 교체)
  이미지 프롬프트 블록    → 제거
  다음 편 예고            → 제거

[내부 링크 원칙 — 필수]
외부 블로그 CTA를 넣지 마세요. 본문 말미에는 블로그스팟 내부 관련글로 이어질 수 있는 문맥형 문장을 1개만 넣으세요.

[학습 요약 박스 — FAQ 바로 앞 필수]
<div style="background:#1E3A5F;color:white;padding:20px 24px;border-radius:10px;margin:24px 0;"><p style="font-size:1.05em;font-weight:700;margin:0 0 12px 0;">📚 오늘의 AI 학습 요약</p><ol style="padding-left:20px;margin:0 0 12px 0;"><li style="margin:6px 0;">[핵심1]</li><li style="margin:6px 0;">[핵심2]</li><li style="margin:6px 0;">[핵심3]</li></ol><p style="margin:0;font-size:0.9em;opacity:0.85;">이 글을 끝까지 읽은 당신은 이미 상위 독자입니다.</p></div>

[AdSense 슬롯]
섹션 3개 이상 시, 두 번째 섹션 끝에 주석 삽입: <!-- AD_SLOT_1 -->

[라벨] 2~5개만 사용. 영문·한국어 혼합 가능. 금지: 인공지능, 딥러닝, 빅데이터, IT, 프로그래밍, 경영, 머신러닝

[출력 JSON — 다른 텍스트 없이]
{
  "title": "SEO 최적화 제목 (25~45자, 도구명+숫자, 도구명 첫 단어 금지, 감정 유지형 마무리)",
  "content": "HTML 본문 (<div> 래퍼 포함, 3500자 이상, 이미지 태그 금지)",
  "tags": ["라벨1", "라벨2", "라벨3"],
  "meta_description": "질문형+답변 축약 120자 이내",
  "faq_items": [
...
class AiContentService:
    ...
    def generate_from_naver_post(self, post: "NaverPost") -> str:  # noqa: F821
        """네이버 블로그 원고 기반으로 블로그스팟 최적화 버전 생성. work_item_id 반환."""
        from blogspot_automation.services.naver_blog_service import NaverPost as _NaverPost

        content_type = _determine_next_content_type(self.work_item_repository)
        reader_level = _select_reader_level()
        logger.info("네이버 재작성 content_type=%s reader_level=%s", content_type, reader_level)

        # 도구명 추출 (제목 앞부분)
        tool_name = re.split(r"[,—–\-]", post.title)[0].strip()[:25]

        # 외부 검색형 LLM 의존성 제거: 도구 가격·기능은 제공 원고/공식 확인 필요 문구로만 처리한다.
        tool_info = ""
        logger.info("도구 정보 외부 조회 스킵: %s", tool_name)

        _section_combo, _section_guide = _content_type_section_guide(content_type, reader_level, tool_name)
        _cta = _cta_html(reader_level)
        _today_str = datetime.now(timezone.utc).strftime("%Y년 %m월 %d일")

        _tool_info_block = (
            f"[{tool_name} 공식 정보 — 가격·기능은 반드시 이 정보만 사용, 임의 창작 금지]\n{tool_info}\n\n"
            if tool_info else
            f"[주의] '{tool_name}' 공식 정보를 조회하지 못했음. 가격·수익 수치 창작 금지. 불확실한 수치는 '공식 사이트 확인 필요'로 대체.\n\n"
        )
        ...
        user_prompt = (
            ...
            f"\n{_section_guide}\n"
            f"\n[마무리 CTA HTML]\n{_cta}\n"
            f"\n위 네이버 원고를 기반으로 블로그스팟 SEO·AEO·GEO·SGE 최적화 버전을 재구성하세요.\n"
            f"원고 핵심 인사이트는 유지하되 완전히 새로운 문장으로 재구성, 원문 복사 금지.\n"
            f"반드시 faq_items(5개)를 JSON에 포함하고, hashtags는 최대 3개만 포함하세요.\n"
            f"\n[학습성·정보성 강제 — 절대 준수]\n"
            f"독자가 글을 다 읽고 '오늘 새로 배운 게 있다'고 느끼게 만드는 것이 최우선입니다.\n"
            f"- 본문 텍스트 4000자 이상 필수 (시스템 프롬프트 3500 기준보다 더 풍부하게)\n"
            f"- 새로 배우는 정보 최소 5개 명시: 구체 사례 2개 + 단계별 실행법 3단계 이상 + 흔한 실수와 해결책 2가지 + 비교/평가 기준\n"
            f"- 모든 주장에 근거 1개 이상: 수치, 사례, 출처 명시\n"
            f"- '~할 수 있습니다', '~좋습니다' 같은 모호한 일반론 금지 → 구체 상황·수치·고유명사로 대체\n"
            f"- 원고가 짧으면 LLM 사전지식으로 깊이를 더하라 (창작 금지 항목인 가격·수익은 제외)\n"
            f"\n[결론 섹션 강제 — 단순 요약 금지]\n"
            f"마무리 CTA 직전에 반드시 다음 형식의 결론 섹션을 넣을 것:\n"
            f"- <h2>이것만 기억하세요</h2>: 글 전체의 핵심 인사이트 1문장 (독자가 캡처해 보관할 수 있을 정도로 압축)\n"
            f"- <h2>지금 바로 할 일 3가지</h2>: 독자가 글을 닫자마자 실행할 수 있는 액션 3개 (각 1문장 + 왜 해야 하는지 근거 1문장)\n"
            f"- '도움이 되셨길', '감사합니다' 같은 빈 마무리 금지\n"
            f"\n[제목 강화]\n"
            f"- 25~45자, 숫자 또는 구체 상황 묘사 1개 이상 포함\n"
            f"- '총정리', '완벽 가이드', '모든 것', '진짜 후기' 같은 진부한 표현 금지\n"
            f"- 독자가 SERP에서 클릭할 이유가 제목 한 줄에 드러나야 함\n"
        )
        ...
        for attempt in range(_MAX_RETRIES + 1):
            ...
            ai_result = self._call_llm(current_prompt, system_prompt=NAVER_REWRITE_SYSTEM_PROMPT)
            ai_result.article_html = post_process_html(ai_result.article_html)

            qa_result = validate_content(
                title=ai_result.title_candidates[0] if ai_result.title_candidates else "",
                content=ai_result.article_html,
                tags=ai_result.labels,
                tool=None,
                banned_tags=banned,
            ...
            if qa_result["passed"]:
                logger.info("QA 통과 (attempt %d)", attempt + 1)
                break
            last_errors = qa_result["errors"]
            logger.warning("QA 실패 (attempt %d): %s", attempt + 1, last_errors)

        if ai_result is None:
            raise RuntimeError("네이버 원고 재작성 LLM 호출 실패")

        # 라벨 정리
        _category_labels = ["AI Tools", "AI활용법", "Productivity", "Side Hustle", "AI부업", "No-Code", "자동화", "AI리뷰"]
        _merged = list(dict.fromkeys((ai_result.labels or []) + _category_labels))
        ai_result.labels = normalize_labels([lb for lb in _merged if lb not in banned])
        ai_result.hashtags = normalize_hashtags(ai_result.hashtags)

        # work_item 생성 및 저장
        wid = f"naver-{uuid4().hex[:12]}"
        timestamp = datetime.now(timezone.utc).isoformat()
        work_item = BlogWorkItem(
            id=wid,
            created_at=timestamp,
            updated_at=timestamp,
            content_pillar="naver_rewrite",
            topic_title=post.title,
            primary_keyword=tool_name,
            publish_status=PublishStatus.PLANNED.value,
            selected_topic=post.title,
            source_count=3,
            source_quality_status="sufficient",
            source_articles=[{"title": post.title, "link": post.link, "provider_name": "naver_blog"}],
            title_candidates=ai_result.title_candidates,
            final_title=ai_result.title_candidates[0] if ai_result.title_candidates else post.title,
            meta_description=ai_result.meta_description,
            labels=ai_result.labels,
            article_html=ai_result.article_html,
            faq_items=ai_result.faq_items,
            hashtags=ai_result.hashtags if hasattr(ai_result, "hashtags") else [],
            content_type=content_type,
            content_density_status="dense",
            notes=f"naver_rewrite from {post.link}",
        )
        self.work_item_repository.upsert(work_item)

        # Brief 저장 (blog_package_service.build_package()가 필수로 요구)
        brief_record = BriefRecord(
            work_item_id=wid,
            created_at=timestamp,
            updated_at=timestamp,
            brief_summary=ai_result.meta_description,
            final_angle=ai_result.title_candidates[0] if ai_result.title_candidates else post.title,
            target_reader="AI 기술/교육 관심 독자",
            one_line_hook=ai_result.one_line_hook,
            faq_items=ai_result.faq_items,
            estimated_time_to_start=ai_result.estimated_time_to_start,
        ...
        self.brief_repository.upsert(brief_record)

        _record_tool_used("naver_rewrite", content_type=content_type, reader_level=reader_level, section_combo=_section_combo)
        logger.info("네이버 재작성 완료: wid=%s title=%s", wid, work_item.final_title)
        return wid
