...
# AI 패턴별 실전 활용 시나리오 (use_cases) — "이럴 때 이렇게" 3개씩.
_AI_USE_CASES: dict[str, list[dict[str, str]]] = {
    "ai_work_time_savings": [
        {"상황": "매주 비슷한 형식의 주간보고를 쓴다", "활용": "지난주 보고서를 예시로 주고 '같은 형식으로 이번 주 내용 채워줘'라고 시키면 뼈대 작성 시간이 사라진다."},
        {"상황": "받은 영문 메일·자료를 빠르게 파악해야 한다", "활용": "전문을 붙여넣고 '핵심 3줄 요약 + 내가 답해야 할 것'을 요청해 읽는 시간을 줄인다."},
        {"상황": "회의가 끝나고 정리가 밀린다", "활용": "녹취·메모를 넣고 '결정사항/할 일(담당자 표)/다음 안건'으로 정리시켜 5분 안에 공유한다."},
    ],
    "ai_tool_comparison": [
        {"상황": "팀에 도입할 AI를 골라야 한다", "활용": "후보 도구에 같은 업무를 시켜 결과·검수시간을 비교한 표를 만들면 설득 자료가 된다."},
        {"상황": "무료로 버틸 수 있는지 모르겠다", "활용": "무료 플랜으로 2주 써보고 막히는 지점(횟수·모델·파일)을 기록해 유료 전환 여부를 판단한다."},
        {"상황": "도구가 너무 많아 고민된다", "활용": "내 핵심 업무 1개를 기준 작업으로 정하고 그 작업만 잘하는 도구를 고른다."},
    ],
    "ai_automation_workflow": [
        {"상황": "반복 메일 분류에 시간을 뺏긴다", "활용": "규칙이 명확한 분류부터 자동화하되, 애매한 건은 사람 확인 큐로 빼는 구조로 설계한다."},
        {"상황": "여러 도구를 오가며 복붙한다", "활용": "입력→처리→출력 형식을 먼저 고정하고 파일럿으로 10건만 돌려 오류율을 본 뒤 확대한다."},
        {"상황": "자동화가 자꾸 깨진다", "활용": "검수 루프(샘플 점검)를 넣고, 실패 시 알림받는 안전장치를 먼저 만든다."},
    ],
    "ai_prompt_recipe": [
        {"상황": "결과가 매번 달라 불안하다", "활용": "출력 형식을 고정한 템플릿을 저장해두고 값만 바꿔 재사용하면 품질이 일정해진다."},
        {"상황": "보고서 초안이 늘 막막하다", "활용": "역할·목적·자료·출력형식·제약 5줄 템플릿에 회의 메모만 넣어 80점 초안을 받는다."},
        {"상황": "AI가 자꾸 지어낸다", "활용": "'확실하지 않으면 추측하지 말고 확인 필요로 표시' 한 줄을 프롬프트에 넣어 환각을 줄인다."},
    ],
    "ai_tool_review": [
        {"상황": "후기만 보고 사기엔 불안하다", "활용": "무료로 내 실제 업무 1개를 시켜 결과물 검수 시간을 재보면 광고 후기보다 정확하다."},
        {"상황": "회사에서 쓸 수 있을지 모르겠다", "활용": "데이터 처리·학습 제외 설정과 회사 정책을 먼저 확인하고, 민감정보는 빼고 테스트한다."},
        {"상황": "유료 전환 타이밍이 고민이다", "활용": "무료로 막히는 지점(횟수·고급 모델·파일)을 기록해 그 기능이 내게 꼭 필요할 때만 전환한다."},
    ],
    "ai_model_update": [
        {"상황": "전에 길이 제한으로 못 넣던 문서가 있다", "활용": "긴 보고서·계약서 전문을 새 모델에 넣어 요약·검토가 한 번에 되는지 가장 먼저 확인한다."},
        {"상황": "코드 디버깅에 시간을 많이 쓴다", "활용": "막혔던 코드와 에러 로그를 새 모델에 주고 원인·수정안을 받아 이전 모델과 비교한다."},
        {"상황": "새 기능(이미지·음성)이 궁금하다", "활용": "실제 업무 예시 하나로 새 기능을 테스트해 내 워크플로에 쓸 곳이 있는지 찾는다."},
    ],
    "ai_search_change": [
        {"상황": "AI 답변에 내 글이 안 잡힌다", "활용": "각 섹션 첫 문장에 핵심 답을 배치하고 정의·비교를 표/박스로 구조화해 인용 확률을 높인다."},
        {"상황": "오래된 글의 노출이 떨어진다", "활용": "업데이트 날짜·출처를 갱신하고 질문형 FAQ를 추가해 신선도·신뢰 신호를 보강한다."},
        {"상황": "정보성 글의 클릭이 줄었다", "활용": "단순 정보 대신 직접 테스트·데이터·관점을 더해 AI가 인용하고 사람도 클릭할 깊이를 만든다."},
    ],
    "ai_blog_growth": [
        {"상황": "글은 많은데 조회수가 안 는다", "활용": "검색 의도가 명확한 주제(‘X 하는 법’, ‘A vs B’) 위주로 전환하고 얕은 글은 통합한다."},
        {"상황": "AI 초안이 다 비슷해 보인다", "활용": "내 경험·수치·스크린샷을 한 단락이라도 넣어 차별화하고 발행 전 체크리스트로 거른다."},
        {"상황": "수익이 안 난다", "활용": "도구 비교·요금제 같은 구매의도 키워드 글 비중을 늘려 광고 단가(RPM)를 높인다."},
    ],
    "ai_comparison": [
        {"상황": "글쓰기 중심 업무다", "활용": "문장 자연스러움·한국어 품질이 강한 쪽을 무료로 비교해 고른다."},
        {"상황": "코딩·분석 중심 업무다", "활용": "추론·긴 컨텍스트에 강한 쪽에 같은 작업을 시켜 검수 시간으로 결정한다."},
        {"상황": "둘 다 비슷해 보인다", "활용": "동일 입력을 양쪽에 넣어 다듬는 시간이 짧은 쪽이 내게 맞는 도구다."},
    ],
    "ai_risk_security": [
        {"상황": "사내 자료를 요약하고 싶다", "활용": "기밀·개인정보를 지운 일반화 버전만 입력하거나 데이터 보호되는 기업용 옵션을 쓴다."},
        {"상황": "결과를 바로 제출하려 한다", "활용": "숫자·인용·출처 등 핵심 사실 2~3개를 원문으로 검증한 뒤 사용한다."},
        {"상황": "AI 이미지·문구를 만들었다", "활용": "상표·저작권 충돌 여부를 확인하고 표현을 손본 뒤 게시한다."},
    ],
    "ai_beginner_guide": [
        {"상황": "뭐부터 해야 할지 모르겠다", "활용": "자주 쓰는 이메일 다듬기 같은 작은 일 하나를 무료 도구에 맡겨 첫 성공을 경험한다."},
        {"상황": "결과가 이상하게 나온다", "활용": "요청에 역할·목적·형식을 한 줄씩 적어 더 구체적으로 다시 시킨다."},
        {"상황": "용어가 어렵다", "활용": "모르는 단어는 ‘초등학생도 알게 예시로 설명해줘’라고 AI에게 직접 물어본다."},
    ],
}


# AI 패턴별 심화 FAQ (기존 3개에 2개 추가 → 5개). 실전 노하우 위주의 저장·공유용 답변.
_AI_EXTRA_FAQ: dict[str, list[dict[str, str]]] = {
    "ai_work_time_savings": [
        {"Q": "어떤 업무부터 AI에 맡기는 게 효율적인가요?",
         "A": "매주 반복되는 텍스트 업무(보고서·이메일·요약)부터 시작하세요. 형식이 정해져 있어 검수가 빠르고 시간 절감이 즉각적입니다. 반면 창의적 기획이나 사실관계가 중요한 업무는 AI를 보조로만 쓰고 사람이 판단해야 합니다."},
        {"Q": "AI를 써도 오히려 시간이 더 걸린다면 무엇이 문제인가요?",
         "A": "보통 두 가지입니다. 맞지 않는 업무에 쓰거나, 완성본을 기대해 계속 다시 시키는 경우죠. '80점 초안을 받아 20점만 내가 채운다'로 기준을 바꾸고, 검수가 5분을 넘으면 프롬프트의 형식·조건 줄을 고치세요."},
    ],
    "ai_tool_review": [
        {"Q": "도구 성능을 객관적으로 비교하려면 어떻게 하나요?",
         "A": "같은 입력(내 실제 업무 1건)을 넣고 ①요청 형식을 지켰는가 ②사실이 정확한가 ③검수에 걸린 시간 세 가지를 기록하세요. 벤치마크 순위보다 내 업무 기준의 검수 시간이 훨씬 정확한 지표입니다."},
        {"Q": "광고성 후기와 진짜 후기를 어떻게 구분하나요?",
         "A": "단점·한계를 구체적으로 말하는지, 무료로 어디까지 되고 어디서 막히는지를 명시하는지, 직접 테스트한 화면이나 수치가 있는지를 보세요. 장점만 나열하고 단점이 없으면 광고일 확률이 높습니다."},
    ],
    "ai_model_update": [
        {"Q": "업데이트 직후 정보가 너무 많은데 무엇부터 봐야 하나요?",
         "A": "딱 두 가지만 먼저 보세요. '무엇이 새로 가능해졌나(능력 변화)'와 '요금·사용 한도 변화'입니다. 벤치마크 순위나 SNS 반응은 나중 일이고, 내 업무에 영향을 주는 능력 변화가 핵심입니다."},
        {"Q": "굳이 새 모델로 안 바꾸고 구버전을 써도 되나요?",
         "A": "글쓰기·요약 위주라면 구버전으로 충분한 경우가 많습니다. 긴 문서 처리·추론·코드처럼 전에 막히던 능력이 내 업무에 꼭 필요할 때만 전환하세요. 업데이트가 곧 '바꿔야 함'을 의미하지는 않습니다."},
    ],
    "ai_search_change": [
        {"Q": "AI 검색 때문에 블로그를 접어야 하나요?",
         "A": "아닙니다. 단순 정보성 글의 클릭은 줄 수 있지만, 직접 경험·데이터·관점이 담긴 깊은 글은 AI가 인용하고 사람도 클릭합니다. 인용되면 출처 링크로 노출돼 신뢰와 유입이 오히려 늘 수 있습니다."},
        {"Q": "AI가 인용하기 좋은 글의 공통점은 무엇인가요?",
         "A": "①질문에 첫 문장으로 바로 답하고 ②정의·비교를 표나 박스로 구조화하며 ③최신 날짜와 출처를 명시하고 ④한 문단으로 추출 가능한 요약을 둡니다. 키워드 반복보다 명확성과 신뢰 신호가 중요합니다."},
    ],
    "ai_blog_growth": [
        {"Q": "AI로 쓴 글도 애드센스 승인이 되나요?",
         "A": "됩니다. 다만 검수 없이 양산한 얇은 글은 '가치 없는 콘텐츠'로 거절될 수 있습니다. 사람의 경험·검증이 더해진 깊이 있는 글과 필수 페이지(소개·문의·개인정보처리방침)를 갖추는 것이 먼저입니다."},
        {"Q": "조회수를 늘리려면 글을 매일 올려야 하나요?",
         "A": "개수보다 검색 의도 충족이 우선입니다. 깊이 있는 글 하나가 얕은 글 열 개보다 오래 트래픽을 모읍니다. 발행 후 관련 글을 내부 링크로 묶고, 오래된 글은 주기적으로 업데이트하는 편이 효과적입니다."},
    ],
    "ai_comparison": [
        {"Q": "벤치마크 점수가 높은 도구를 고르면 되나요?",
         "A": "벤치마크는 시험 점수라 내 업무와 다를 수 있습니다. 같은 작업을 양쪽에 시켜 검수 시간으로 비교하는 게 정확합니다. 글쓰기·코딩·분석 등 용도별로 강점이 다르므로 '내 핵심 업무'를 기준으로 고르세요."},
        {"Q": "무료끼리, 유료끼리 비교할 때 기준이 다른가요?",
         "A": "무료는 '사용량·기능 제한'을, 유료는 '월 비용 대비 절감되는 시간'을 봅니다. 무료로 충분하면 유료는 실제로 한계에 부딪힐 때 전환하면 됩니다. 가격은 자주 바뀌니 공식 페이지로 확인하세요."},
    ],
    "ai_risk_security": [
        {"Q": "이미 회사 자료를 AI에 입력했는데 어떻게 해야 하나요?",
         "A": "①해당 서비스 설정에서 대화 삭제·학습 제외를 적용하고 ②회사 보안 담당에 보고하며 ③민감도가 높으면 관련 계정·자료 접근을 점검하세요. 다음부터는 기밀·개인정보를 지운 익명화 버전만 입력하는 것이 원칙입니다."},
        {"Q": "AI가 만든 결과물의 저작권은 안전한가요?",
         "A": "모호한 영역입니다. 생성물이 기존 저작물과 유사할 수 있어 이미지·문구·상표는 충돌 여부를 확인하고 표현을 손본 뒤 사용하세요. 상업적으로 쓰기 전에는 해당 서비스 약관의 권리·이용 조항을 반드시 확인해야 합니다."},
    ],
    "ai_beginner_guide": [
        {"Q": "유료 결제부터 해야 잘 쓸 수 있나요?",
         "A": "아닙니다. 요약·번역·이메일 초안 같은 작업은 무료로 충분히 시작할 수 있습니다. 무료로 익숙해진 뒤 사용량이나 기능에 막힐 때 유료를 고려해도 늦지 않습니다."},
        {"Q": "AI가 자꾸 엉뚱한 답을 해요. 왜 그런가요?",
         "A": "요청이 모호한 경우가 대부분입니다. '누구처럼(역할) + 무엇을(목적) + 어떤 형식으로' 세 가지를 한 줄씩 적어 다시 시키면 크게 좋아집니다. 그리고 중요한 사실은 항상 직접 확인하세요."},
    ],
}
...
class SlotFillerService:
    ...
    def fill_slots(
        ...
        # AI 패턴 공통: 실전 활용 시나리오 주입 (필수 슬롯 아님 → fill_rate 무영향)
        if pattern_id in _AI_USE_CASES and not _is_filled(slots.get("use_cases")):
            slots["use_cases"] = _AI_USE_CASES[pattern_id]
            fill_strategy["use_cases"] = "lookup"

        # AI 패턴 공통: 심화 FAQ 2개 추가(중복 제외) → FAQ 5개로 확장
        if pattern_id in _AI_EXTRA_FAQ:
            base_faq = list(slots.get("faq") or []) if isinstance(slots.get("faq"), list) else []
            existing_q = {str(f.get("Q", "")).strip() for f in base_faq if isinstance(f, dict)}
            for extra in _AI_EXTRA_FAQ[pattern_id]:
                if str(extra.get("Q", "")).strip() not in existing_q:
                    base_faq.append(extra)
            slots["faq"] = base_faq
            ...
            "prompt_block": [
                {
                    "label": "이메일 정중하게 다듬기",
                    "prompt": (
                        "당신은 비즈니스 이메일 교정 담당자입니다.\n"
                        "아래 초안을 정중하고 간결한 업무 이메일로 다듬어 주세요.\n\n"
                        "[이메일 초안 붙여넣기]\n\n"
                        "조건: 3문단 이내, 공손한 어조, 핵심 요청을 첫 문단에 배치."
                    ),
                },
                {
                    "label": "긴 자료 핵심 요약",
                    "prompt": (
                        "당신은 핵심만 뽑는 편집자입니다.\n"
                        "아래 자료를 의사결정에 필요한 정보 위주로 요약해 주세요.\n\n"
                        "[자료 붙여넣기]\n\n"
                        "출력 형식: 한 줄 요약 → 핵심 포인트 5개 → 확인 필요 항목 2개."
                    ),
                },
                {
                    "label": "회의록 정리",
                    "prompt": (
                        "당신은 회의록을 정리하는 실무자입니다.\n"
                        "아래 메모를 결정사항과 할 일 중심으로 정리해 주세요.\n\n"
                        "[회의 메모 붙여넣기]\n\n"
                        "출력 형식: 결정사항 / 담당자별 할 일(표) / 다음 회의 안건."
                    ),
                },
            ],
            "misconceptions": [
    ...
    def _build_ai_prompt_recipe(
        ...
        slots: dict[str, Any] = {
            ...
            "prompt_block": [
                {
                    "label": "① 만능 기본 템플릿 (역할·목적·입력·형식·제약)",
                    "prompt": (
                        "당신은 [직무/전문 역할]입니다.\n"
                ...
                },
                {
                    ...
                    "prompt": (
                        "당신은 비즈니스 이메일 교정 담당자입니다.\n"
                        "아래 요점으로 정중하고 간결한 업무 이메일을 작성해 주세요.\n\n"
                        "받는 사람: [예: 거래처 담당자]\n"
                        "전하려는 요점: [핵심 메시지 1~3개]\n\n"
                        "조건: 3문단 이내, 공손하지만 군더더기 없는 어조, 핵심 요청을 첫 문단에 배치, "
                        "제목도 함께 제안."
                    ),
                },
                {
                    ...
                    "prompt": (
                        ...
                        "아래 메모를 결정사항과 할 일 중심으로 정리해 주세요.\n\n"
                        "[회의 메모/녹취 붙여넣기]\n\n"
                        "출력 형식: ① 결정사항 ② 담당자별 할 일(표: 담당자·할 일·기한) ③ 다음 회의 안건\n"
                        "조건: 메모에 없는 내용은 추측하지 말고 '미정'으로 표시."
                    ),
                },
                {
                    "label": "⑥ 기획·아이디어 브레인스토밍",
                    "prompt": (
                        "당신은 [분야] 기획자입니다.\n"
                        "[목표/문제]에 대한 아이디어를 발산해 주세요.\n\n"
                        "제약/조건: [예산·기간·타깃 등]\n\n"
                        "출력 형식: 서로 다른 방향의 아이디어 7개 → 각 아이디어마다 한 줄 설명 + 예상 리스크 1개\n"
                        "조건: 뻔한 아이디어 대신 관점이 다른 것 위주로, 실행 난이도(상/중/하) 표시."
                    ),
                },
                {
                    "label": "⑦ 자연스러운 번역·현지화",
                    "prompt": (
                        "당신은 [영어→한국어] 전문 번역가입니다.\n"
                        "아래 문장을 의미는 유지하되 한국어로 자연스럽게 번역해 주세요.\n\n"
                        "[원문 붙여넣기]\n\n"
                        "조건: 직역투 금지, 대상 독자는 [일반 독자/실무자], 전문용어는 괄호로 원어 병기, "
                        "어색한 부분은 의역하고 그 이유를 한 줄로 덧붙여 주세요."
                    ),
                },
            ],
            "real_criterion": (
            ...
            "faq": [
                ...
                {
                    "Q": "원하는 형식이 안 나올 때는 어떻게 고치나?",
                    "A": (
                        "설명을 늘리지 말고 '출력 형식' 줄을 더 구체적으로 바꾼다. 예를 들어 '표로 정리'보다 "
                        "'표 | 열: 항목·설명·예시'처럼 열 이름까지 지정하면 형식이 안정된다. 한 번에 좋은 예시를 보여주는 "
                        "'예시 1개 먼저 주기(one-shot)'도 형식 일치에 효과적이다."
                    ),
                },
                {
                    "Q": "대화가 길어지면 답이 산으로 가는데?",
                    "A": (
                        "맥락이 누적돼 흐려진 것이다. 작업이 바뀌면 새 대화를 시작하고, 필요한 자료만 다시 붙여넣는 게 깔끔하다. "
                        "또는 '지금까지 내용은 무시하고 아래 기준으로 다시 작성'이라고 명시해 맥락을 리셋한다."
                    ),
                },
            ],
            "hashtags": [
            ...
            "prompt_block": [
                {
                    "label": "복붙용 기본 요청 양식",
                    "prompt": (
                        "당신은 [역할: 예) 친절한 글쓰기 도우미]입니다.\n"
                        "[하고 싶은 일: 예) 아래 글을 쉽게 다듬어 주세요]\n\n"
                        "[내용 붙여넣기]\n\n"
                        "조건: 쉬운 말로, 3문단 이내."
                    ),
                },
                {
                    "label": "모르는 용어 쉽게 묻기",
                    "prompt": "‘[모르는 용어]’를 초등학생도 이해할 수 있게 한 문단으로 쉽게 설명해 주세요. 예시도 하나 들어 주세요.",
                },
            ],
            "real_criterion": (
