...
class SlotFillerService:
    ...
    def _build_ai_model_update(
        ...
        slots: dict[str, Any] = {
            "hook_opening": (
                "새 AI 모델이 나왔다는 소식이 뜨면 '내 업무가 진짜 달라지나'부터 궁금해진다. "
                "벤치마크 점수와 데모 영상은 화려하지만, 실제 체감 차이는 업무 유형에 따라 크게 갈린다. "
                "발표 내용 중 확인된 사실과 아직 검증되지 않은 기대를 구분해서 봐야 한다. "
                "지금 바뀐 게 무엇이고, 그게 나에게 영향을 주는지부터 정리하는 것이 먼저다."
            ),
            "yomi_judgment": (
                "모델 업데이트의 핵심은 '버전 숫자'가 아니라 내 반복 업무에서 체감되는 변화다. "
                "벤치마크 향상은 평균값일 뿐, 글쓰기·요약처럼 이미 잘 되던 작업은 차이가 작을 수 있다. "
                "추론·코딩·긴 문서 처리처럼 한계가 있던 영역에서 변화가 크다. "
                "지금 쓰는 도구의 모델이 자동 업데이트되는지부터 확인하고, 내 핵심 업무 하나로 직접 비교해 보라."
            ),
            "misconceptions": [
                {"착각": "새 모델이 나오면 모든 작업이 더 좋아진다", "실제": "이미 잘 되던 작업은 차이가 미미하고, 한계가 있던 영역에서만 체감된다"},
                {"착각": "벤치마크 1위면 내 업무에도 최고다", "실제": "벤치마크는 특정 시험 점수다. 내 실제 업무로 직접 비교해야 안다"},
                {"착각": "업데이트되면 요금이 그대로다", "실제": "고급 모델은 유료 플랜·사용량 제한이 함께 바뀌는 경우가 많다"},
            ],
            "real_criterion": (
                "1단계: 무엇이 바뀌었는지 공식 발표에서 확인 — 모델명, 적용 범위, 무료/유료 제공 여부를 공식 채널로 본다.\n"
                "2단계: 확인된 사실과 추측을 구분 — 벤치마크·데모는 참고치이고, 실사용 후기는 시간이 지나야 쌓인다.\n"
                "3단계: 내 핵심 업무 하나로 직접 비교 — 같은 입력을 이전/신규에 넣어 결과와 검수 시간을 비교한다."
            ),
            "quick_decision_table": [
                {"내 상황": "글쓰기·요약 위주다", "할 일": "체감 차이가 작을 수 있으니 무료 범위에서 먼저 비교 후 판단"},
                {"내 상황": "코딩·분석·긴 문서 처리", "할 일": "변화가 큰 영역 — 내 실제 작업으로 직접 테스트할 가치 있음"},
                {"내 상황": "유료 전환을 고민 중", "할 일": "달라진 요금·사용량 제한을 공식 페이지에서 확인 후 결정"},
                {"내 상황": "도구가 자동 업데이트된다", "할 일": "출력 형식이 달라질 수 있으니 자주 쓰는 프롬프트 결과부터 점검"},
            ],
            "actions": [
                {"번호": 1, "행동": "공식 발표로 변경점 확인", "설명": "모델명·적용 범위·무료 제공 여부를 공식 채널에서 직접 확인한다"},
                {"번호": 2, "행동": "내 업무로 직접 비교", "설명": "같은 입력을 이전/신규 모델에 넣어 결과와 검수 시간을 비교한다"},
                {"번호": 3, "행동": "요금·제한 변화 점검", "설명": "고급 모델 사용량·유료 조건이 바뀌었는지 확인하고 필요할 때만 전환한다"},
            ],
            "faq": [
                {"Q": "새 모델로 꼭 바꿔야 하나요?", "A": "내 업무에서 체감 차이가 있을 때만 바꾸면 됩니다. 글쓰기·요약 위주라면 기존으로도 충분한 경우가 많습니다."},
                {"Q": "벤치마크 점수를 믿어도 되나요?", "A": "참고치로만 보세요. 벤치마크는 특정 시험 점수라 내 실제 업무 성능과 다를 수 있습니다. 직접 비교가 가장 정확합니다."},
                {"Q": "무료로도 새 모델을 쓸 수 있나요?", "A": "초기에는 유료·사용량 제한이 있는 경우가 많습니다. 무료 제공 범위는 공식 발표에서 확인해야 합니다."},
            ],
