...
def cover_image_url_from_env(*, content_type: str = "", topic_group: str = "") -> str:
    for key in _cover_image_env_keys(content_type=content_type, topic_group=topic_group):
...
def _cover_image_env_keys(*, content_type: str = "", topic_group: str = "") -> tuple[str, ...]:
    keys: list[str] = []
...
def ensure_cover_image_html(
    ...
    *,
    ...
    safe_alt = escape(" ".join((alt_text or title or "news cover image").split()), quote=True)
    block = (
